...
These examples make the architecture concepts in Chapter 4 executable without coupling the handbook to a specific LLM provider. They use only the Python standard library and intentionally use deterministic stand-ins where a real model gateway would normally sit.
...
They are **not** production LLM clients. Chapters covering model integration and RAG will introduce provider SDKs and infrastructure-specific implementations later.
...
```bash
python 01_prompt_contract.py
python 02_zero_and_few_shot.py
# ...and so on
```
...
- Schema validation proves output shape, not truth or authorization.
- Injection detection in these examples is intentionally simplistic; production systems need layered controls.
